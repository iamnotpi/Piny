...
class DataLoaderLite:
    def __init__(self, batch_size, sequence_length, process_rank, num_processes, split, shuffle=True, seed=42):
    ...
    def shuffle_batches(self, seed):
        B, T = self.B, self.T
        num_batches = len(self.tokens) // (B * T)
        torch.manual_seed(seed)
        indices = torch.randperm(num_batches)
        batches = [self.tokens[i * B * T:(i + 1) * B * T + 1] for i in indices if (i + 1) * B * T + 1 < len(self.tokens)]
        self.tokens = torch.cat(batches)
    ...
    def next_batch(self):
        ...
        if self.current_position + B * T * self.process_rank + 1 > len(self.tokens):
            ...
            offset = 0
            if self.shuffle:
                self.shuffle_batches(seed=self.seed)
                offset = 1
            self.current_position = self.process_rank * B * T + offset
        return X, Y
